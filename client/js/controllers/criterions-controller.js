...
$http.get('/api/criterions/' + $scope.projectID).success(function(data) {
  ...
  $scope.draggableCriterions = $scope.criterions; 
  document.getElementById("ratioZ").value  = $scope.project.ratioZ;
  document.getElementById("ratioZ1").value = $scope.project.ratioZ1;
  document.getElementById("ratioZ2").value = $scope.project.ratioZ2;
  document.getElementById("ratioZ3").value = $scope.project.ratioZ3;
  document.getElementById("ratioZIntervalMin").value = $scope.project.ratioZMin;
  document.getElementById("ratioZIntervalMax").value = $scope.project.ratioZMax;
  document.getElementById("ratioZIntervalResult").value = $scope.project.ratioZInterval;
  decimalPlaces = $scope.project.decimals;
  if(decimalPlaces == 1){
    document.getElementById("nDecimals").innerHTML += '.x';
  }else if(decimalPlaces == 2){
    document.getElementById("nDecimals").innerHTML += '.xx';
  }
  if($scope.project.ratioOption == "oneValue"){
    document.getElementById("oneValue").checked = true;
  }else if($scope.project.ratioOption == "interval"){
      document.getElementById("interval").checked = true;
  }else if($scope.project.ratioOption == "threeValues"){
    document.getElementById("threeValues").checked = true;
  }else{
    // Do nothing because no option was selected last time the method was executed
  }
  if($scope.project.criteria.length == 0){
      document.getElementById('sectionsCriteria').style.backgroundColor = '#ff3333';
      $scope.criteriaDone = false;
...
var weightFirstTime = true;
$scope.showWeightMethod = function(){
  $('#loading').show();
  // If it's the first time to show the weight method, add drag/drop and show method
  if(weightFirstTime){
    $scope.draggableCriterions = $scope.criterions; 
    document.getElementById("weightMethod").style.display = 'block';
    console.log('Show weight method. Adding drag and drop events...');
    $timeout( function(){ addDragDrop(); }, 1000); 
    weightFirstTime = false;
    $('#loading').hide();
  }else{
    // If it's not the first time calling the weight method and the user added or removed a criterion, then refresh page to update the weight method
...
// Reload the page if a criterion is added or removed and if the weight method was called once
$scope.hideWeightMethod = function(){
  $('#loading').show();
  if(weightFirstTime){
    // Nothing happens... 
    $('#loading').hide();
...
$scope.updateRatioZ = function(id) {
  console.log('Saving ratio Z changes...');
  $timeout( function(){
    if(id == "ratioZ1"){
      $scope.project.ratioZ1 = document.getElementById("ratioZ1").value;
    }else if(id == "ratioZ2"){
      $scope.project.ratioZ2 = document.getElementById("ratioZ2").value;
    }else if(id == "ratioZ3"){
      $scope.project.ratioZ3 = document.getElementById("ratioZ3").value;
    }else if(id == "ratioZ"){
      $scope.project.ratioZ = document.getElementById("ratioZ").value;
    }else if(id == "ratioZIntervalMin"){
      $scope.project.ratioZMin = document.getElementById("ratioZIntervalMin").value;
    }else if(id == "ratioZIntervalMax"){
      $scope.project.ratioZMax = document.getElementById("ratioZIntervalMax").value;
    }else if(id == "ratioZIntervalResult"){
      $scope.project.ratioZInterval = document.getElementById("ratioZIntervalResult").value;
    } else{
      //Do nothing
    } 
    var project = $scope.project
...
$scope.updateDecimals = function(){
  $timeout( function(){
    console.log('Saving decimals changes...');
    $scope.project.decimals = decimalPlaces;
    var project = $scope.project
    var i = $scope.project._id;
    $http.put('/api/project/' + i, project).success(function(response) {
    });
...
$scope.updateRatioOption = function(){
    console.log('Saving ratio option chosen...');
    if(document.getElementById("oneValue").checked == true){
      $scope.project.ratioOption = "oneValue";
    }else if(document.getElementById("interval").checked == true){
      $scope.project.ratioOption = "interval";
    }else if(document.getElementById("threeValues").checked == true){
      $scope.project.ratioOption = "threeValues";
    }else{
      $scope.project.ratioOption = "";
    }
    var project = $scope.project
    var i = $scope.project._id;
    $http.put('/api/project/' + i, project).success(function(response) {
...
function decreaseRatio(id){
    if(id == "ratioZ1"){
        ...
        if(number == 1 || (number-1) < 1 ){
            ...
            document.getElementById("ratioZIntervalResult").value = 1;
        }else{
            document.getElementById("ratioZIntervalMin").value = parseFloat(number) - 1;
            document.getElementById("ratioZIntervalResult").value = document.getElementById("ratioZIntervalMin").value;
        }
    }else if(id == "ratioZIntervalMax"){
...
function weightResults(){
    ...
    document.getElementById("exportBtn1").style.display = '';
